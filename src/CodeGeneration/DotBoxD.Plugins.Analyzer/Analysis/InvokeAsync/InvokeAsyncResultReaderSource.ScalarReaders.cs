...
internal sealed partial class InvokeAsyncResultReaderSource
{
    private string EnsureSingleValueReader()
    {
        ...
        _helpers.AppendLine("            {");
        _helpers.AppendLine("                throw new global::System.NotSupportedException(\"InvokeAsync F64 result cannot be represented as System.Single without overflow.\");");
        _helpers.AppendLine("            }");
        _helpers.AppendLine();
    ...
    {
        var key = "enum:" + TypeName(enumType);
        if (_readers.TryGetValue(key, out var existing))
        {
            return existing;
        }
    ...
    private void AppendEnumRangeGuard(string minimum, string maximum)
    {
        _helpers.Append("            if (__value < ").Append(minimum).Append(" || __value > ").Append(maximum)
            .AppendLine(")");
        _helpers.AppendLine("            {");
        _helpers.AppendLine("                throw new global::System.NotSupportedException(\"InvokeAsync enum result is outside the target enum underlying range.\");");
        _helpers.AppendLine("            }");
    }
