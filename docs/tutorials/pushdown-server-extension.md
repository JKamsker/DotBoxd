# Tutorial: Pushdown — ship a server-side batch operation
...
For N monsters that is N remote calls, N serialization passes, and N chances for latency to dominate. You cannot add `KillMonsters(...)` to the host — it is already released. This is exactly the situation Pushdown is designed for (see [`docs/concepts/pushdown.md`](../concepts/pushdown.md)).
