...
- **A trusted channel, not a sandbox.** A provided service is callable by *any* peer on the channel, so
  ...
  author logic is Kernels / [Pushdown](pushdown.md), not Services.
...
subset, prefer the [event pipeline (RunLocal)](../tutorials/event-pipeline-runlocal.md) — `Where` /
...
server-side batch, prefer [Pushdown](pushdown.md) — the batch runs as verified, capability-gated,
fuel-metered IR.
