...
RunLocal and Pushdown run author logic as validated, fuel-metered [kernel IR](../concepts/kernels.md) (never trusted CLR code);
Services is a trusted channel you guard at the transport or application layer. See
[Choosing a mode](../index.md#choosing-a-mode) for the full comparison and the
[glossary](../reference/glossary.md) for unfamiliar terms — the three quickstarts below map one-to-one
...
See [concepts/services.md](../concepts/services.md), or [build it step by step](../tutorials/first-service.md).
...
See the GameServer sample or [concepts/kernels.md](../concepts/kernels.md). For a concrete runnable version
...
`SandboxHost` steps above — [build it step by step](../tutorials/event-pipeline-runlocal.md).
...
`MonsterKillerKernel` server extension. See [concepts/pushdown.md](../concepts/pushdown.md), or
[build it step by step](../tutorials/pushdown-server-extension.md).
...
[What the run prints](../examples/gameserver-walkthrough.md#what-the-run-prints).
...
Ready for an end-to-end walkthrough? Work through the [tutorials](../tutorials/index.md).
