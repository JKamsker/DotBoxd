...
public class ReviewedBehaviorGapTests
{
    ...
    [Fact]
    public void InheritedBaseInterfaces_WithSyncAndAsyncCollision_FiresSHARPC004()
    {
        const string source = """
            using ShaRPC.Core.Attributes;
    ...
    public void GenericUnsupportedMethod_PreservesAllConstraintKindsOnStub()
    {
        ...
        runResult.Diagnostics.Where(d => d.Id == "SHARPC002").Should().HaveCount(5);
        var proxy = runResult.Results.Single().GeneratedSources
            .Single(g => g.HintName.EndsWith("IGenericConstraints.ShaRpcProxy.g.cs"))
            .SourceText.ToString();
