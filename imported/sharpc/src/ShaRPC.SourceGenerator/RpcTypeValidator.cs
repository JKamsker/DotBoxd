...
internal static class RpcTypeValidator
{
    ...
    public static string? GetUnsupportedSubServicePayloadReason(
        ...
        CancellationToken ct,
        RpcTypeValidationCache? cache = null) =>
        ContainsShaRpcServiceInterface(type, ct, cache)
    ...
    private static bool ContainsShaRpcServiceInterface(
        ...
        cache is null
            ? SubServicePayloadInspector.ContainsShaRpcServiceInterface(type, ct)
            : cache.ContainsShaRpcServiceInterface(type, ct);
