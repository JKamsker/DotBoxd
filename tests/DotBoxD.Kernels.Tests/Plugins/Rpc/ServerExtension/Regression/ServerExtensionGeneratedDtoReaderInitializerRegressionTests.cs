...
public sealed partial class ServerExtensionGeneratedDtoReaderRegressionTests
{
    ...
    private const string InitializerHelperComputedDtoSource = """
        using DotBoxD.Kernels;
        using DotBoxD.Kernels.Sandbox;
        using DotBoxD.Plugins;
        using DotBoxD.Plugins.Runtime;
        ...
        public sealed class Profile
        {
            ...
            private int ComputeScore() => this.Health + this.Rank;
        }

        [ServerExtension(typeof(IRemoteWorldControl), "profile-init-helper")]
        public sealed partial class ProfileKernel
        {
            [ServerExtensionMethod(typeof(IRemoteWorldControl))]
            public Profile Read(int x, HookContext ctx)
    ...
    [Fact]
    public void Direct_extension_reconstructs_an_initializer_dto_with_a_helper_computed_get_only_member()
    {
        var assembly = PluginAnalyzerGeneratedPackageFactory.CreateAssembly(InitializerHelperComputedDtoSource);
        var control = CreateControl(
            assembly,
            "profile-init-helper",
            KernelRpcBinaryCodec.EncodeValue(KernelRpcValue.Record(
