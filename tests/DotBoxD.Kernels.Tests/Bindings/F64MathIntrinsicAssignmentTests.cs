...
public sealed class F64MathIntrinsicAssignmentTests
{
    ...
    [Theory]
    [MemberData(nameof(Cases))]
    public async Task Compiled_f64_math_direct_return_bindings_match_interpreter(string expression, double expected)
    {
        var result = await ExecuteDirectReturnAsync(
            expression,
            SandboxPolicyBuilder.Create().Build(),
            new SandboxExecutionOptions { Mode = ExecutionMode.Compiled, AllowFallbackToInterpreter = false });
    ...
    [Theory]
    [MemberData(nameof(Cases))]
    public async Task Compiled_f64_math_direct_return_charges_host_call_limit(string expression, double _)
    {
        var result = await ExecuteDirectReturnAsync(
            expression,
            SandboxPolicyBuilder.Create().WithMaxHostCalls(0).Build(),
    ...
    private static async Task<SandboxExecutionResult> ExecuteDirectReturnAsync(
        string expression,
        SandboxPolicy policy,
        SandboxExecutionOptions options)
    {
        ...
        var module = await host.ImportJsonAsync($$"""
        {
          "id": "f64-math-intrinsic-direct-return",
          "version": "1.0.0",
          "functions": [
            {
              "id": "main",
