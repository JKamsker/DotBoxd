...
public sealed class CompiledSideEffectCapabilityParityTests
{
    ...
    private static string CapabilityParity_MessageModuleJson(string id)
        => CapabilityParity_MessageModuleJsonWithTarget(id, "player-1");

    private static string CapabilityParity_MessageModuleJsonWithTarget(string id, string targetId)
        => $$"""
        {
          "id": "{{id}}",
          "version": "1.0.0",
    ...
    private static SandboxPolicy CapabilityParity_GameWritePolicy()
        => new(
            "game-write",
    ...
    private static string CapabilityParity_CounterModuleJson(string id)
        => $$"""
        {
          "id": "{{id}}",
          "version": "1.0.0",
    ...
    /// <summary>Thread-safe counter used as the custom binding's external state.</summary>
    private sealed class Capability_Counter
    {
        private int _total;
    ...
    /// <summary>
    /// Re-declares InMemoryPluginMessageSink locally to avoid name collision when test files
    /// are combined. Delegates to the real implementation.
    /// </summary>
    private sealed class Capability_InMemoryPluginMessageSink : IPluginMessageSink
    {
