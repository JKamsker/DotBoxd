...
/// call to <c>ThrowIfOverflowed</c> throws <see cref="ShaRpcProtocolException"/> —
...
public sealed class StreamingOverflowCascadeTests
{
    ...
    /// <c>RpcStreamManager.cs</c> throws <see cref="ShaRpcProtocolException"/> regardless
    ...
    public async Task GetRegisteredInbound_ForValidReceiver_IsNotBlockedByTombstoneOverflow()
    {
        ...
        try
        {
            ...
            // permanently latched to true, this throws ShaRpcProtocolException(
            ...
            Assert.Null(ex); // RED: ShaRpcProtocolException is thrown here today.
        }
        finally
        {
            RpcDiagnostics.Error -= OnError;
